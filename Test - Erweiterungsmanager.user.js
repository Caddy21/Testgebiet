    ...
    function renderMissingExtensions(buildings) {
        ...
        buildingGroups = {}; // Initialisierung der globalen Variable
    ...
    for (const { building, missingExtensions } of group) {
        for (const extension of missingExtensions) {
            ...
            if (!isExtensionLimitReached(building, extension.id)) {
                ...
                builtCount++;
                updateProgress(builtCount, totalExtensions);
            }
        }
    }
}
...
function updateProgress(builtCount, totalExtensions) {
    const progressDiv = document.getElementById('progress-div');
    if (progressDiv) {
        progressDiv.textContent = `Erweiterungen gebaut: ${builtCount} von ${totalExtensions}`;
    }
}

// Anpassung der Funktion "showCurrencySelectionForAll", um die neue Funktion zu verwenden
function showCurrencySelectionForAll(groupKey) {
    const group = buildingGroups[groupKey];
